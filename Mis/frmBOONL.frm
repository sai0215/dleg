...
Begin VB.Form frmBOONL 
   ...
   Begin VB.CommandButton cmdAdd 
      BackColor       =   &H00C0FFC0&
      Caption         =   "&Add"
      DisabledPicture =   "frmBOONL.frx":0306
      DownPicture     =   "frmBOONL.frx":0838
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   9.75
         Charset         =   0
 ...
 If ValidateData = True Then
  ...
  If Mid(Trim(cboMediaType.Text), 1, 3) = "Onl" Then
    ...
    Set rs = db.OpenRecordset(Sqlqry1, dbOpenDynaset)
     ...
     Set ws = DBEngine.Workspaces(0)
     Set db = ws.OpenDatabase(App.Path & "\misov.mdb")
     Sqlqry = "Update docu_mas set doc_no='" & lblserialno & "' where doc_type='ONL'"
     ws.BeginTrans
     db.Execute (Sqlqry)
     ws.CommitTrans
...
Private Sub populateMedia()
    ...
    Set ws = DBEngine.Workspaces(0)
    Set db = ws.OpenDatabase(App.Path & "\misov.mdb")
    Sqlqry = "Select * from Media where media_type='Online' Order by Media_Type"
    Set rs = db.OpenRecordset(Sqlqry, dbOpenDynaset)
    If rs.RecordCount = 0 Then
         Exit Sub
    Else
...
Set ws = DBEngine.Workspaces(0)
Set db = ws.OpenDatabase(App.Path & "\misov.mdb")
Sqlqry = "Select DOC_no from DOCU_MAS WHERE DOC_TYPE='ONL'"
Set rs = db.OpenRecordset(Sqlqry, dbOpenDynaset)
If rs.RecordCount = 0 Then
   MsgBox "Document type 'ONL' not found"
   Exit Sub
Else
  ...
  If ValidateData = True Then
    ...
    Set rs = db.OpenRecordset(Sqlqry1, dbOpenDynaset)
     If rs.RecordCount = 0 Then
       Sqlqry1 = " Insert into dumBo_TRAcinbo values('" & Val(lblserialno.Caption) & "','" & cboyear.Text & "','" _
                                     ...
                                     & Format(txtCinDateFrom.TextWithMask, "mm/DD/yyyy") & "#,#" _
                                     & Format(txtCinDateTo.TextWithMask, "mm/DD/yyyy") & "#,'" _
                                     & findfirstfixup(Trim(txtboref.Text)) & "','" _
   ...
   Else
        ...
        Else
            ...
            rs.MoveFirst
            Do Until rs.EOF
                '     MsgBox Format(rs!DATETO, "MM/DD/YYYY")
                '     MsgBox Format(rs!DATEFROM, "MM/DD/YYYY")
                MSFlexcin.AddItem rs!sub_Media & Chr(9) & Format(rs!DATEFROM, "DD/MM/YYYY") & Chr(9) & Format(rs!Dateto, "DD/MM/YYYY") & Chr(9) & rs!Day & Chr(9) & rs!Length & Chr(9) & rs!Description & Chr(9) & rs!mat_code & Chr(9) & rs!Type & Chr(9) & rs!tra_amount
   ...
   i = MsgBox(" Are you sure .. ! You want to Remove this transaction", vbInformation + vbYesNo)
    If i = vbYes Then
     With MSFlexmag
        ...
        txtissueno = .Text
        .Col = 1
        txtdate.TextWithMask = Trim(.Text)
        .Col = 2
        txtPage = .Text
        .Col = 3
        cbospace = .Text
        ...
        Sqlqry1 = "Select * from dumBo_TRAmagbo where  Issue_no ='" & Trim(txtissueno) & "' and tdate=#" & Format(txtissdate.TextWithMask, "MM/DD/YYYY") & "# and Page ='" & Trim(txtPage) & "' and space='" & Trim(cbospace.Text) & "' and tdate =#" & Format(txtissdate.TextWithMask, "mm/dd/yyyy") & "# and tra_amount =" & Val(txtamountmag) & ""
        Set rs1 = db.OpenRecordset(Sqlqry1, dbOpenDynaset)
        If rs1.RecordCount <> 0 Then
       ' MsgBox Sqlqry1
        rs1.MoveLast
