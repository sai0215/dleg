...
Begin VB.Form frmBankPaymentAddition 
   BackColor       =   &H00FFFFFF&
   Caption         =   "Bank Payment - Addition"
   ClientHeight    =   8775
   ClientLeft      =   -120
   ClientTop       =   330
   ClientWidth     =   12060
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.CommandButton cmdClear 
         ...
         Left            =   5880
         MaskColor       =   &H0080C0FF&
         Picture         =   "frmBankPaymentAddition.frx":0000
         Style           =   1  'Graphical
         TabIndex        =   15
         Top             =   7800
         Width           =   975
      ...
      Begin VB.CommandButton cmdBack 
         ...
         Left            =   6840
         MaskColor       =   &H0080C0FF&
         Picture         =   "frmBankPaymentAddition.frx":0442
         Style           =   1  'Graphical
         TabIndex        =   16
         Top             =   7800
         Width           =   975
      ...
      Begin VB.CommandButton cmdSave 
         ...
         Left            =   4920
         MaskColor       =   &H0080C0FF&
         Picture         =   "frmBankPaymentAddition.frx":0884
         Style           =   1  'Graphical
         TabIndex        =   14
         Top             =   7800
         Width           =   975
   ...
   End
End
Attribute VB_Name = "frmBankPaymentAddition"
...
If ValidateData = True Then
   ...
   If Val(txtTtlAmount.Text) = Val(lblTtlAmount.Caption) Then
            ...
            Set ws = DBEngine.Workspaces(0)
            Set db = ws.OpenDatabase(App.Path & "\misov.mdb")
               Sqlqry = " Insert into bpmt_mas values('" & lblVoucNo & "','BPT','" _
                                             & Format(txtdate.TextWithMask, "dd/mm/yyyy") & "','" _
                                             & UCase(Trim(txtPaidTo)) & "','" _
                                             & Mid(cboBankCode, 1, 6) & "','" _
                                             & findfirstfixup(Mid(cboBankCode, 10, 35)) & "','" _
            ...
            Set rs = db.OpenRecordset(Sqlqry1, dbOpenDynaset)
             If rs.RecordCount = 0 Then
              ...
              Else
                 ...
                 Set ws = DBEngine.Workspaces(0)
                 Set db = ws.OpenDatabase(App.Path & "\misov.mdb")
                 Sqlqry3 = "Insert into bpmt_tra values('" & rs!vouc_no & "','" & rs!vouc_type & "','" _
                                             & Trim(rs!tDate) & "','" _
                                             & rs!acct_code & "','" _
            ...
            Set ws = DBEngine.Workspaces(0)
            Set db = ws.OpenDatabase(App.Path & "\misov.mdb")
            Sqlqry = "Update docu_mas set doc_no='" & lblVoucNo & "' where doc_type='BPT'"
            ws.BeginTrans
            db.Execute (Sqlqry)
            ws.CommitTrans
  ...
  If X = vbYes Then
   If Ctype = "DHS" Then
        ...
        CrystalReport1.SelectionFormula = "{Bpmt_tra.Vouc_no}=" & Val(lblVoucNo.Caption) - 1 & ""
...
Private Sub AutoIncrementVoucher()
    Set ws = DBEngine.Workspaces(0)
    Set db = ws.OpenDatabase(App.Path & "\misov.mdb")
    Sqlqry = "Select DOC_no from DOCU_MAS WHERE DOC_TYPE='BPT'"
    Set rs = db.OpenRecordset(Sqlqry, dbOpenDynaset)
    If rs.RecordCount = 0 Then
       MsgBox "Document type 'BPT' not found"
       Exit Sub
    Else
    ...
    If i = vbYes Then
     With MSFlexGrid1
      ...
      Sqlqry = "Select acct_Code,acct_name from Acct_mas where acct_code='" & txtaccode & "' order by acct_code"
        ...
        Sqlqry1 = "Delete * from dumbpmt1 where Acct_Code='" & txtaccode & "' and description ='" & txtdesc & "' and amount =" & Val(txtAmount) & ""
        ws.BeginTrans
        db.Execute Sqlqry1
        ws.CommitTrans
...
Private Sub txtAmount_LostFocus()
  ...
  End If
   ...
   If ValidateData = True Then
    ...
    End If
    If rs.RecordCount = 0 Then
       Sqlqry = " Insert into dumbpmt1 values('" & lblVoucNo & "','BPT','" _
                                     & Format(txtdate.TextWithMask, "dd/mm/yyyy") & "','" _
                                     & accd & "','" _
      ...
      Else
       ...
       Sqlqry = " Insert into dumbpmt1 values('" & lblVoucNo & "','BPT','" _
                                     ...
                                     & Val(Trim(txtConvRate)) & "," _
                                     & Val(Trim(txtAmount)) & "," _
                                     & Val(Val(txtAmount) * Val(txtConvRate)) & ")"
