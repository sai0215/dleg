...
Begin VB.Form frmCashPaymentAddition 
   AutoRedraw      =   -1  'True
   BackColor       =   &H00FFFFC0&
   Caption         =   "Cash Payment Addition"
   ClientHeight    =   8775
   ClientLeft      =   0
   ClientTop       =   240
   ClientWidth     =   12060
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.CommandButton cmdClear 
         ...
         Height          =   855
         Left            =   4560
         Picture         =   "frmCashPayment.frx":0000
         Style           =   1  'Graphical
         TabIndex        =   11
         Top             =   7440
         Width           =   1095
      ...
      Begin VB.CommandButton cmdBack 
         ...
         Height          =   855
         Left            =   5640
         Picture         =   "frmCashPayment.frx":0442
         Style           =   1  'Graphical
         TabIndex        =   12
         Top             =   7440
         Width           =   1095
      ...
      Begin VB.CommandButton cmdSave 
         ...
         Height          =   855
         Left            =   3480
         Picture         =   "frmCashPayment.frx":0884
         Style           =   1  'Graphical
         TabIndex        =   10
         Top             =   7440
         Width           =   1095
   ...
   End
End
Attribute VB_Name = "frmCashPaymentAddition"
    ...
    If i = vbYes Then
     With MSFlexGrid1
       ...
       End If
        ...
        Sqlqry1 = "Delete * from dumCpmt1 where Acct_Code='" & txtaccode & "' and description ='" & txtdesc & "' and amount =" & Val(txtAmount) & ""
        ws.BeginTrans
        db.Execute Sqlqry1
        ws.CommitTrans
...
con = 1
 ...
 If ValidateData = True Then
   If Val(txtTtlAmount.Text) = lblTtlAmount Then
    ...
    Set ws = DBEngine.Workspaces(0)
    Set db = ws.OpenDatabase(App.Path & "\misov.mdb")
       Sqlqry = " Insert into cpmt_mas values('" & lblVoucNo & "','CPT','" _
    ...
    Set rs = db.OpenRecordset(Sqlqry1, dbOpenDynaset)
      ...
      Else
         ...
         Set ws = DBEngine.Workspaces(0)
         Set db = ws.OpenDatabase(App.Path & "\misov.mdb")
         Sqlqry3 = "Insert into CPMT_TRA values('" & rs!vouc_no & "','" & rs!vouc_type & "','" _
                                     & Trim(rs!tDate) & "','" _
                                     & rs!acct_code & "','" _
     ...
     Set ws = DBEngine.Workspaces(0)
     Set db = ws.OpenDatabase(App.Path & "\misov.mdb")
     Sqlqry = "Update docu_mas set doc_no='" & lblVoucNo & "' where doc_type='CPT'"
     ws.BeginTrans
     db.Execute (Sqlqry)
     ws.CommitTrans
  ...
  If X = vbYes Then
   If ctype = "DHS" Then
        ...
        CrystalReport1.SelectionFormula = "{Cpmt_tra.Vouc_no}=" & Val(lblVoucNo.Caption) - 1 & ""
...
Private Sub AutoIncrementVoucher()
    Set ws = DBEngine.Workspaces(0)
    Set db = ws.OpenDatabase(App.Path & "\misov.mdb")
    Sqlqry = "Select DOC_no from DOCU_MAS WHERE DOC_TYPE='CPT'"
    Set rs = db.OpenRecordset(Sqlqry, dbOpenDynaset)
    If rs.RecordCount = 0 Then
       MsgBox "Document type 'CPT' not found"
       Exit Sub
    Else
...
Dim acname As String
  ...
  End If
    ...
    End If
    If rs.RecordCount = 0 Then
       Sqlqry = " Insert into dumcpmt1 values('" & lblVoucNo & "','CPT','" _
                                     & Format(txtdate.TextWithMask, "dd/mm/yyyy") & "','" _
                                     & accd & "','" _
