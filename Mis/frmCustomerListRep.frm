...
Begin VB.Form frmCustomerListRep 
   BackColor       =   &H00FFFFFF&
   ...
   Begin VB.Frame Frame1 
      BackColor       =   &H00FFFFFF&
      Caption         =   "        Statement of Agencies Outstanding in USD      "
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   11.25
         Charset         =   0
      ...
      Begin VB.Frame Frame4 
         ...
         Begin VB.CommandButton cmdAging 
            ...
            Height          =   855
            Left            =   1680
            Picture         =   "frmCustomerListRep.frx":0000
            Style           =   1  'Graphical
            TabIndex        =   1
            Top             =   240
            Width           =   1095
         ...
         Begin VB.CommandButton cmdDisplay 
            ...
            Height          =   855
            Left            =   600
            Picture         =   "frmCustomerListRep.frx":0442
            Style           =   1  'Graphical
            TabIndex        =   0
            Top             =   240
            Width           =   1095
         ...
         Begin VB.CommandButton cmdClear 
            ...
            Height          =   855
            Left            =   2760
            Picture         =   "frmCustomerListRep.frx":0544
            Style           =   1  'Graphical
            TabIndex        =   2
            Top             =   240
            Width           =   1095
         ...
         Begin VB.CommandButton cmdBack 
            ...
            Height          =   855
            Left            =   3840
            Picture         =   "frmCustomerListRep.frx":0986
            Style           =   1  'Graphical
            TabIndex        =   3
            Top             =   240
            Width           =   1095
   ...
   End
End
Attribute VB_Name = "frmCustomerListRep"
...
End Sub
 ...
 ' Agency List for Customere List Report in USD
 ...
 ' Startin
          ...
          If IsNull(rs!op_DHS) = True Then
            Opbal = Opbal
          Else
            Opbal = Opbal + (rs!op_DHS / convertion)
          End If
         cno = Trim(rs!agentname)
                
      ' Sqlqry1 = "select sum(tra_amount) from crpt_tra where ( tdate< #" & DateValue(Format(txtdatefrom.textwithmask, "dd/mm/yyyy")) & "# and acct_name='" & findfirstfixup(rs!agentname) & "')"
        Sqlqry1 = "select sum(tra_amount) from crpt_tra where ( tdate< #" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# and tcurrency ='USD' and acct_name='" & findfirstfixup(Trim(cno)) & "')"
        ...
        Set rs1 = db.OpenRecordset(Sqlqry1, dbOpenDynaset)
        If IsNull(rs1.Fields(0)) = False Then opcrpt = opcrpt + (rs1.Fields(0) / convertion)
               
      ' Cash Payment before From date
      ' Sqlqry2 = "select sum(tra_amount) from cpmt_tra where ( tdate< #" & DateValue(Format(txtdatefrom.textwithmask, "dd/mm/yyyy")) & "# and acct_name='" & findfirstfixup(rs!agentname) & "')"
        Sqlqry2 = "select sum(tra_amount) from cpmt_tra where ( tdate< #" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# and tcurrency ='USD' and acct_name='" & findfirstfixup(Trim(cno)) & "')"
        ...
        Set rs2 = db.OpenRecordset(Sqlqry2, dbOpenDynaset)
        If IsNull(rs2.Fields(0)) = False Then opcpmt = opcpmt + (rs2.Fields(0) / convertion)
            
        ' Bank Receipt before From date
        Sqlqry3 = "select Sum(tra_amount) from brpt_tra where (tdate< #" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# and tcurrency ='USD' and acct_name='" & findfirstfixup(Trim(cno)) & "')"
        ...
        Set rs3 = db.OpenRecordset(Sqlqry3, dbOpenDynaset)
        If IsNull(rs3.Fields(0)) = False Then Opbrpt = Opbrpt + (rs3.Fields(0) / convertion)
        
        
        ' Bank Payment before From date
        
        Sqlqry4 = "select Sum(tra_amount) from bpmt_tra where (tdate< #" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# AND TCURRENCY ='USD' and acct_name='" & findfirstfixup(rs!agentname) & "')"
        Set rs4 = db.OpenRecordset(Sqlqry4, dbOpenDynaset)
        If IsNull(rs4.Fields(0)) = False Then Opbpmt = rs4.Fields(0)
        
        Sqlqry4 = "select Sum(tra_amount) from bpmt_tra where (tdate< #" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# AND TCURRENCY ='DHS' and acct_name='" & findfirstfixup(rs!agentname) & "')"
        Set rs4 = db.OpenRecordset(Sqlqry4, dbOpenDynaset)
        If IsNull(rs4.Fields(0)) = False Then Opbpmt = Opbpmt + (rs4.Fields(0) / convertion)
        
        
       'Pdc Receipts before From date
        Sqlqry5 = "Select sum(tra_amount) from prpt_mas1 where Cheque_dt<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# and not isnull(posting_dt) AND TCURRENCY ='USD' and acct_name='" & findfirstfixup(rs!agentname) & "'"
        Set rs6 = db.OpenRecordset(Sqlqry5, dbOpenDynaset)
        If IsNull(rs6.Fields(0)) = False Then Opprpt = rs6.Fields(0)
        
        Sqlqry5 = "Select sum(tra_amount) from prpt_mas1 where Cheque_dt<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# and not isnull(posting_dt) AND TCURRENCY ='DHS' and acct_name='" & findfirstfixup(rs!agentname) & "'"
        Set rs6 = db.OpenRecordset(Sqlqry5, dbOpenDynaset)
        If IsNull(rs6.Fields(0)) = False Then Opprpt = Opprpt + (rs6.Fields(0) / convertion)
      ...
      'Pdc Payments before From Date
        ...
        If rs7.RecordCount <> 0 Then
         rs7.MoveFirst
          Do Until rs7.EOF
           Sqlqry8 = "Select sum(tra_amount) from Ppmt_tra where Vouc_no=" & Val(rs7!vouc_no) & " and acct_name='" & findfirstfixup(rs!agentname) & "'"
        ...
        If rs7.RecordCount <> 0 Then
         rs7.MoveFirst
          Do Until rs7.EOF
           Sqlqry8 = "Select sum(tra_amount) from Ppmt_tra where Vouc_no=" & Val(rs7!vouc_no) & " and acct_name='" & findfirstfixup(rs!agentname) & "'"
           Set rs8 = db.OpenRecordset(Sqlqry8, dbOpenDynaset)
           If IsNull(rs8.Fields(0)) = False Then Opppmt = Opppmt + (rs8.Fields(0) / convertion)
           rs7.MoveNext
          Loop
        End If
       ...
       ' Journal Debit tra_amount before From Date
        Sqlqry9 = "select Sum(tra_damount) from Jrnl_tra  where (tdate<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# and tcurrency ='USD' and acct_name='" & findfirstfixup(rs!agentname) & "' and dc_code ='D' ) "
        Set rs9 = db.OpenRecordset(Sqlqry9, dbOpenDynaset)
        If IsNull(rs9.Fields(0)) = False Then opjbd = rs9.Fields(0)
        
        Sqlqry9 = "select Sum(tra_damount) from Jrnl_tra  where (tdate<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# and tcurrency ='DHS' and acct_name='" & findfirstfixup(rs!agentname) & "' and dc_code ='D' ) "
        Set rs9 = db.OpenRecordset(Sqlqry9, dbOpenDynaset)
        If IsNull(rs9.Fields(0)) = False Then opjbd = opjbd + (rs9.Fields(0) / convertion)
        
                         
       ' Journal Credit tra_amount before From Date
        Sqlqry10 = " select sum(tra_camount) From Jrnl_tra where (tdate<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# and tcurrency='USD' and acct_name='" & findfirstfixup(rs!agentname) & "' and dc_code ='C'); "
        Set rs10 = db.OpenRecordset(Sqlqry10, dbOpenDynaset)
        If IsNull(rs10.Fields(0)) = False Then Opjbc = rs10.Fields(0)
        
        Sqlqry10 = " select sum(tra_camount) From Jrnl_tra where (tdate<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# and tcurrency='DHS' and acct_name='" & findfirstfixup(rs!agentname) & "' and dc_code ='C'); "
        Set rs10 = db.OpenRecordset(Sqlqry10, dbOpenDynaset)
        If IsNull(rs10.Fields(0)) = False Then Opjbc = Opjbc + (rs10.Fields(0) / convertion)
        
       ' Opening balance  debit note (credit) before From Date
        SQLQRY11 = "select Sum(tra_amount) from debt_mas where (tdate<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# and tcurrency='USD' and acct_name='" & findfirstfixup(rs!agentname) & "');"
        Set rs11 = db.OpenRecordset(SQLQRY11, dbOpenDynaset)
        If IsNull(rs11.Fields(0)) = False Then opdbntcr = rs11.Fields(0)
        
        SQLQRY11 = "select Sum(tra_amount) from debt_mas where (tdate<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# and tcurrency='DHS' and acct_name='" & findfirstfixup(rs!agentname) & "');"
        Set rs11 = db.OpenRecordset(SQLQRY11, dbOpenDynaset)
        If IsNull(rs11.Fields(0)) = False Then opdbntcr = opdbntcr + (rs11.Fields(0) / convertion)
        
         
       ' Opening balance debit note (debit)  before From Date
        SQLQRY12 = "select Sum(tra_amount) from debt_mas where tdate<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# and tcurrency='USD' and cust_name='" & findfirstfixup(rs!agentname) & "'"
        Set rs12 = db.OpenRecordset(SQLQRY12, dbOpenDynaset)
        If IsNull(rs12.Fields(0)) = False Then opdbntdb = rs12.Fields(0)
        
        SQLQRY12 = "select Sum(tra_amount) from debt_mas where tdate<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# and tcurrency='DHS' and cust_name='" & findfirstfixup(rs!agentname) & "'"
        Set rs12 = db.OpenRecordset(SQLQRY12, dbOpenDynaset)
        If IsNull(rs12.Fields(0)) = False Then opdbntdb = opdbntdb + (rs12.Fields(0) / convertion)
        
        
       ' Opening balance  Credit note (debit) before From Date
        Sqlqry13 = "select Sum(tra_amount) from crdt_mas where tdate<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# AND TCURRENCY ='USD' and acct_name='" & findfirstfixup(rs!agentname) & "'"
        Set rs13 = db.OpenRecordset(Sqlqry13, dbOpenDynaset)
        If IsNull(rs13.Fields(0)) = False Then Opcrntdb = rs13.Fields(0)
         
        Sqlqry13 = "select Sum(tra_amount) from crdt_mas where tdate<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# AND TCURRENCY ='DHS' and acct_name='" & findfirstfixup(rs!agentname) & "'"
        Set rs13 = db.OpenRecordset(Sqlqry13, dbOpenDynaset)
        If IsNull(rs13.Fields(0)) = False Then Opcrntdb = Opcrntdb + (rs13.Fields(0) / convertion)
         
       ' Opening balance credit note (credit)  before From Date
        Sqlqry14 = "select sum(tra_amount) from crdt_mas where tdate<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# AND TCURRENCY='USD' and SUPP_NAME='" & findfirstfixup(rs!agentname) & "'"
        Set rs14 = db.OpenRecordset(Sqlqry14, dbOpenDynaset)
        If IsNull(rs14.Fields(0)) = False Then Opcrntcr = rs14.Fields(0)
        
        Sqlqry14 = "select sum(tra_amount) from crdt_mas where tdate<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# AND TCURRENCY='DHS' and SUPP_NAME='" & findfirstfixup(rs!agentname) & "'"
        Set rs14 = db.OpenRecordset(Sqlqry14, dbOpenDynaset)
        If IsNull(rs14.Fields(0)) = False Then Opcrntcr = Opcrntcr + (rs14.Fields(0) / convertion)
        
              
        ' Opening balance credit Sales  before From Date
        Sqlqry15 = "select Sum(Tra_namount) from BO_mas where INVOICE_date<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "#  and tcurrency='USD' and AGENCY='" & findfirstfixup(rs!agentname) & "' and cancell='N'"
        Set rs15 = db.OpenRecordset(Sqlqry15, dbOpenDynaset)
        If IsNull(rs15.Fields(0)) = False Then Opcrsl = rs15.Fields(0)
        
        Sqlqry15 = "select Sum(Tra_namount) from BO_mas where INVOICE_date<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "#  and tcurrency='DHS' and AGENCY='" & findfirstfixup(rs!agentname) & "' and cancell='N'"
        Set rs15 = db.OpenRecordset(Sqlqry15, dbOpenDynaset)
        If IsNull(rs15.Fields(0)) = False Then Opcrsl = rs15.Fields(0) + (rs15.Fields(0) / convertion)
        
                             
        ' Opening balance credit Purchases  before From Date
        sqlqry16 = "select sum(TRA_amount) from crpr_mas where tdate<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# AND TCURRENCY = 'USD' and SUPP_NAME='" & findfirstfixup(rs!agentname) & "'"
        Set rs16 = db.OpenRecordset(sqlqry16, dbOpenDynaset)
        If IsNull(rs16.Fields(0)) = False Then Opcrpr = rs16.Fields(0)
        
        sqlqry16 = "select sum(TRA_amount) from crpr_mas where tdate<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# AND TCURRENCY = 'DHS' and SUPP_NAME='" & findfirstfixup(rs!agentname) & "'"
        Set rs16 = db.OpenRecordset(sqlqry16, dbOpenDynaset)
        If IsNull(rs16.Fields(0)) = False Then Opcrpr = Opcrpr + (rs16.Fields(0) / convertion)
        ...
        If rs18.RecordCount <> 0 Then
         rs18.MoveFirst
         Do Until rs18.EOF
           If rs18!tcurrency = "USD" Then
                sqlqry19 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs18!vouc_no & "','" & rs18!vouc_type & "','" & Trim(rs18!tDate) & "','" & findfirstfixup(Trim(rs18!Description)) & "'," & 0 & "," & Trim(rs18!tra_amount) & ")"
                ws.BeginTrans
                db.Execute (sqlqry19)
                ws.CommitTrans
           Else
                sqlqry19 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs18!vouc_no & "','" & rs18!vouc_type & "','" & Trim(rs18!tDate) & "','" & findfirstfixup(Trim(rs18!Description)) & "'," & 0 & "," & Val(rs18!tra_amount) / convertion & ")"
                ws.BeginTrans
                db.Execute (sqlqry19)
                ws.CommitTrans
           End If
        ...
        If rs19.RecordCount <> 0 Then
         rs19.MoveFirst
         Do Until rs19.EOF
          If rs19!tcurrency = "USD" Then
            Sqlqry21 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs19!vouc_no & "','" & rs19!vouc_type & "','" & Trim(rs19!tDate) & "','" & findfirstfixup(Trim(rs19!Description)) & "'," & Trim(rs19!tra_amount) & "," & 0 & ")"
            ws.BeginTrans
            db.Execute (Sqlqry21)
            ws.CommitTrans
          Else
            Sqlqry21 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs19!vouc_no & "','" & rs19!vouc_type & "','" & Trim(rs19!tDate) & "','" & findfirstfixup(Trim(rs19!Description)) & "'," & Val(rs19!tra_amount) / convertion & "," & 0 & ")"
            ws.BeginTrans
            db.Execute (Sqlqry21)
            ws.CommitTrans
          End If
        ...
        If rs20.RecordCount <> 0 Then
         rs20.MoveFirst
         Do Until rs20.EOF
           If rs20!tcurrency = "USD" Then
                Sqlqry23 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs20!vouc_no & "','" & rs20!vouc_type & "','" & Trim(rs20!tDate) & "','" & findfirstfixup(Trim(rs20!Description)) & "'," & 0 & "," & Val(rs20!tra_amount) & ")"
                ws.BeginTrans
                db.Execute (Sqlqry23)
                ws.CommitTrans
           Else
                Sqlqry23 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs20!vouc_no & "','" & rs20!vouc_type & "','" & Trim(rs20!tDate) & "','" & findfirstfixup(Trim(rs20!Description)) & "'," & 0 & "," & Val(rs20!tra_amount) / convertion & ")"
                ws.BeginTrans
                db.Execute (Sqlqry23)
                ws.CommitTrans
            End If
        ...
        If rs21.RecordCount <> 0 Then
         rs21.MoveFirst
         Do Until rs21.EOF
           If rs21!tcurrency = "USD" Then
               Sqlqry25 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs21!vouc_no & "','" & rs21!vouc_type & "','" & Trim(rs21!tDate) & "','" & findfirstfixup(Trim(rs21!Description)) & "'," & Val(rs21!tra_amount) & "," & 0 & ")"
               ws.BeginTrans
               db.Execute (Sqlqry25)
               ws.CommitTrans
           Else
              Sqlqry25 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs21!vouc_no & "','" & rs21!vouc_type & "','" & Trim(rs21!tDate) & "','" & findfirstfixup(Trim(rs21!Description)) & "'," & Val(rs21!tra_amount) / convertion & "," & 0 & ")"
               ws.BeginTrans
               db.Execute (Sqlqry25)
               ws.CommitTrans
           End If
       ...
       ' Pdc Receipts after From date and before to date
        ...
        If rs23.RecordCount <> 0 Then
         rs23.MoveFirst
          Do Until rs23.EOF
             If rs23!tcurrency = "USD" Then
                Sqlqry28 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs23!vouc_no & "','" & rs23!vouc_type & "','" & Trim(rs23!Cheque_Dt) & "','" & findfirstfixup(Trim(rs23!Description)) & "'," & 0 & "," & Val(rs23!tra_amount) & ")"
                 ws.BeginTrans
                 db.Execute (Sqlqry28)
                 ws.CommitTrans
             Else
                 Sqlqry28 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs23!vouc_no & "','" & rs23!vouc_type & "','" & Trim(rs23!Cheque_Dt) & "','" & findfirstfixup(Trim(rs23!Description)) & "'," & 0 & "," & Val(rs23!tra_amount) / convertion & ")"
                 ws.BeginTrans
                 db.Execute (Sqlqry28)
                 ws.CommitTrans
             End If
         ...
         Do Until rs24.EOF
            ...
            Set rs25 = db.OpenRecordset(Sqlqry30, dbOpenDynaset)
             If rs25.RecordCount <> 0 Then
               rs25.MoveFirst
                Do Until rs25.EOF
                    If rs25!tcurrency = "USD" Then
                        Sqlqry31 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs25!vouc_no & "','" & rs25!vouc_type & "','" & Trim(rs24!Cheque_Dt) & "','" & findfirstfixup(Trim(rs25!Description)) & "'," & Trim(rs25!tra_amount) & "," & 0 & ")"
                        ws.BeginTrans
                        db.Execute (Sqlqry31)
                        ws.CommitTrans
                    Else
                        Sqlqry31 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs25!vouc_no & "','" & rs25!vouc_type & "','" & Trim(rs24!Cheque_Dt) & "','" & findfirstfixup(Trim(rs25!Description)) & "'," & Val(rs25!tra_amount) / convertion & "," & 0 & ")"
                        ws.BeginTrans
                        db.Execute (Sqlqry31)
                        ws.CommitTrans
                    End If
       ...
       ' Journal Debit after From date and before to date
        ...
        Set rs26 = db.OpenRecordset(Sqlqry32, dbOpenDynaset)
         If rs26.RecordCount <> 0 Then
          rs26.MoveFirst
          Do Until rs26.EOF
           If rs26!tcurrency = "USD" Then
           
                Sqlqry33 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs26!vouc_no & "','" & rs26!vouc_type & "','" & Trim(rs26!tDate) & "','" & findfirstfixup(Trim(rs26!Description)) & "'," & Val(rs26!tra_damount) & "," & 0 & ")"
                ws.BeginTrans
                db.Execute (Sqlqry33)
                ws.CommitTrans
           Else
                Sqlqry33 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs26!vouc_no & "','" & rs26!vouc_type & "','" & Trim(rs26!tDate) & "','" & findfirstfixup(Trim(rs26!Description)) & "'," & Val(rs26!tra_damount) / convertion & "," & 0 & ")"
                ws.BeginTrans
                db.Execute (Sqlqry33)
                ws.CommitTrans
           End If
    ...
    ' Journal Credit after From date and before to date
        ...
        If rs27.RecordCount <> 0 Then
         rs27.MoveFirst
        Do Until rs27.EOF
          If rs27!tcurrency = "USD" Then
            Sqlqry34 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs27!vouc_no & "','" & rs27!vouc_type & "','" & Trim(rs27!tDate) & "','" & findfirstfixup(Trim(rs27!Description)) & "'," & 0 & "," & Val(rs27!tra_Camount) & ")"
            ws.BeginTrans
            db.Execute (Sqlqry34)
            ws.CommitTrans
           Else
            Sqlqry34 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs27!vouc_no & "','" & rs27!vouc_type & "','" & Trim(rs27!tDate) & "','" & findfirstfixup(Trim(rs27!Description)) & "'," & 0 & "," & Val(rs27!tra_Camount) / convertion & ")"
            ws.BeginTrans
            db.Execute (Sqlqry34)
            ws.CommitTrans
           End If
    ...
    ' DebitNote - credit after From date and before to date
        ...
        If rs28.RecordCount <> 0 Then
          rs28.MoveFirst
         Do Until rs28.EOF
          If rs28!tcurrency = "USD" Then
            Sqlqry36 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs28!vouc_no & "','" & rs28!vouc_type & "','" & Trim(rs28!tDate) & "','" & findfirstfixup(Trim(rs28!Description)) & "'," & 0 & "," & Trim(rs28!tra_amount) & ")"
            ws.BeginTrans
            db.Execute (Sqlqry36)
            ws.CommitTrans
          Else
            Sqlqry36 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs28!vouc_no & "','" & rs28!vouc_type & "','" & Trim(rs28!tDate) & "','" & findfirstfixup(Trim(rs28!Description)) & "'," & 0 & "," & Val(rs28!tra_amount) / convertion & ")"
            ws.BeginTrans
            db.Execute (Sqlqry36)
            ws.CommitTrans
          End If
        ...
        If rs29.RecordCount <> 0 Then
          rs29.MoveFirst
         Do Until rs29.EOF
          If rs29!tcurrency = "USD" Then
            Sqlqry38 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs29!vouc_no & "','" & rs29!vouc_type & "','" & Trim(rs29!tDate) & "','" & findfirstfixup(Trim(rs29!Description)) & "'," & Trim(rs29!tra_amount) & "," & 0 & ")"
            ws.BeginTrans
            db.Execute (Sqlqry38)
            ws.CommitTrans
          Else
            Sqlqry38 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs29!vouc_no & "','" & rs29!vouc_type & "','" & Trim(rs29!tDate) & "','" & findfirstfixup(Trim(rs29!Description)) & "'," & Val(rs29!tra_amount) / convertion & "," & 0 & ")"
            ws.BeginTrans
            db.Execute (Sqlqry38)
            ws.CommitTrans
          End If
        ...
        Set rs30 = db.OpenRecordset(Sqlqry38, dbOpenDynaset)
         If rs30.RecordCount <> 0 Then
           rs30.MoveFirst
         Do Until rs30.EOF
           If rs30!tcurrency = "USD" Then
                Sqlqry39 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs30!vouc_no & "','" & rs30!vouc_type & "','" & Trim(rs30!tDate) & "','" & findfirstfixup(Trim(rs30!Description)) & "'," & 0 & "," & Trim(rs30!tra_amount) & ")"
                ws.BeginTrans
                db.Execute (Sqlqry39)
                ws.CommitTrans
           Else
                Sqlqry39 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs30!vouc_no & "','" & rs30!vouc_type & "','" & Trim(rs30!tDate) & "','" & findfirstfixup(Trim(rs30!Description)) & "'," & 0 & "," & Val(rs30!tra_amount) / convertion & ")"
                ws.BeginTrans
                db.Execute (Sqlqry39)
                ws.CommitTrans
           End If
    ...
    ' Credit Note - Debit after From date and before to date
        ...
        If rs31.RecordCount <> 0 Then
         rs31.MoveFirst
        Do Until rs31.EOF
          If rs31!tcurrency = "USD" Then
            Sqlqry41 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs31!vouc_no & "','" & rs31!vouc_type & "','" & Trim(rs31!tDate) & "','" & findfirstfixup(Trim(rs31!Description)) & "'," & Trim(rs31!tra_amount) & "," & 0 & ")"
            ws.BeginTrans
            db.Execute (Sqlqry41)
            ws.CommitTrans
          Else
            Sqlqry41 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs31!vouc_no & "','" & rs31!vouc_type & "','" & Trim(rs31!tDate) & "','" & findfirstfixup(Trim(rs31!Description)) & "'," & Val(rs31!tra_amount) / convertion & "," & 0 & ")"
            ws.BeginTrans
            db.Execute (Sqlqry41)
            ws.CommitTrans
          End If
    ...
    ' Credit Sale after From date and before to date
        ...
        Do Until rs32.EOF
          If rs32!tcurrency = "USD" Then
            Sqlqry43 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs32!serial_no & "','INV','" & Trim(rs32!invoice_date) & "','" & findfirstfixup(Trim(rs32!bo_ref)) & " " & Trim(rs32!media) & "'," & Trim(rs32!tra_namount) & "," & 0 & ")"
            ws.BeginTrans
            db.Execute (Sqlqry43)
            ws.CommitTrans
          Else
            Sqlqry43 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs32!serial_no & "','INV','" & Trim(rs32!invoice_date) & "','" & findfirstfixup(Trim(rs32!bo_ref)) & " " & Trim(rs32!media) & "'," & Val(rs32!tra_namount) / convertion & "," & 0 & ")"
            ws.BeginTrans
            db.Execute (Sqlqry43)
            ws.CommitTrans
          End If
    ...
    ' Credit Purchase after From date and before to date
        ...
        If rs33.RecordCount <> 0 Then
         rs33.MoveFirst
         Do Until rs33.EOF
           If rs33!tcurrency = "USD" Then
                Sqlqry45 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs33!vouc_no & "','" & rs33!vouc_type & "','" & Trim(rs33!tDate) & "','Purchases'," & 0 & "," & Val(rs33!tra_amount) & ")"
                ws.BeginTrans
                db.Execute (Sqlqry45)
                ws.CommitTrans
            Else
                Sqlqry45 = "Insert into CustReport values('" & findfirstfixup(rs!agentname) & "','" & rs33!vouc_no & "','" & rs33!vouc_type & "','" & Trim(rs33!tDate) & "','Purchases'," & 0 & "," & Val(rs33!tra_amount) / convertion & ")"
                ws.BeginTrans
                db.Execute (Sqlqry45)
                ws.CommitTrans
            End If
   ...
   ' Pdc Receipts after From date and before to date
        ...
        If rs35.RecordCount <> 0 Then
         rs35.MoveFirst
            Do Until rs35.EOF
              ttlpdc = ttlpdc + (rs35!tra_amount / convertion)
               rs35.MoveNext
             Loop
        End If
