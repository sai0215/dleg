...
Begin VB.Form frmCustomerRep 
   BackColor       =   &H00FFFFFF&
   Caption         =   "Financial Reports - Agency  statement in USD"
   ClientHeight    =   8775
   ClientLeft      =   -30
   ClientTop       =   300
   ClientWidth     =   12060
   ...
   Begin VB.Frame Frame1 
      BackColor       =   &H00FFFFFF&
      Caption         =   " Agency Statement in USD"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   11.25
         Charset         =   0
      ...
      Begin VB.CommandButton cmdBack 
         ...
         Height          =   855
         Left            =   4200
         Picture         =   "frmCustomerRep.frx":0000
         Style           =   1  'Graphical
         TabIndex        =   5
         Top             =   5640
         Width           =   1575
      ...
      Begin VB.CommandButton cmdClear 
         ...
         Height          =   855
         Left            =   2640
         Picture         =   "frmCustomerRep.frx":0442
         Style           =   1  'Graphical
         TabIndex        =   4
         Top             =   5640
         Width           =   1575
      ...
      Begin VB.CommandButton cmdDisplay 
         ...
         Height          =   855
         Left            =   1080
         Picture         =   "frmCustomerRep.frx":0884
         Style           =   1  'Graphical
         TabIndex        =   3
         Top             =   5640
         Width           =   1575
   ...
   End
End
Attribute VB_Name = "frmCustomerRep"
...
Private Sub cmdDisplay_Click()
     ...
     If ValidateData = True Then
      ...
      ' Sqlqry2 = "select sum(tra_amount) from cpmt_tra where ( tdate< #" & DateValue(Format(txtdatefrom.textwithmask, "dd/mm/yyyy")) & "# and acct_name='" & findfirstfixup(lstcustomers) & "')"
        ...
        Sqlqry4 = "select Sum(tra_amount) from bpmt_tra where (tdate< #" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# AND TCURRENCY ='USD' and acct_name='" & findfirstfixup(lstCustomers) & "')"
       ...
       'Pdc Receipts before From date
        Sqlqry5 = "Select sum(tra_amount) from prpt_mas1 where Cheque_dt<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# and not isnull(posting_dt) AND TCURRENCY ='USD' and acct_name='" & findfirstfixup(lstCustomers) & "'"
      ...
      'Pdc Payments before From Date
       ...
       ' Journal Debit tra_amount before From Date
        Sqlqry9 = "select Sum(tra_damount) from Jrnl_tra  where (tdate<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# and tcurrency ='USD' and acct_name='" & findfirstfixup(lstCustomers) & "' and dc_code ='D' ) "
       ...
       ' Journal Credit tra_amount before From Date
        Sqlqry10 = " select sum(tra_camount) From Jrnl_tra where (tdate<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# and tcurrency='USD' and acct_name='" & findfirstfixup(lstCustomers) & "' and dc_code ='C'); "
       ...
       ' Opening balance  debit note (credit) before From Date
        SQLQRY11 = "select Sum(tra_amount) from debt_mas where (tdate<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# and tcurrency='USD' and acct_name='" & findfirstfixup(lstCustomers) & "');"
       ...
       ' Opening balance debit note (debit)  before From Date
        SQLQRY12 = "select Sum(tra_amount) from debt_mas where tdate<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# and tcurrency='USD' and cust_name='" & findfirstfixup(lstCustomers) & "'"
       ...
       ' Opening balance  Credit note (debit) before From Date
        Sqlqry13 = "select Sum(tra_amount) from crdt_mas where tdate<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# AND TCURRENCY ='USD' and acct_name='" & findfirstfixup(lstCustomers) & "'"
       ...
       ' Opening balance credit note (credit)  before From Date
        Sqlqry14 = "select sum(tra_amount) from crdt_mas where tdate<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# AND TCURRENCY='USD' and SUPP_NAME='" & findfirstfixup(lstCustomers) & "'"
        ...
        Sqlqry15 = "select Sum(Tra_namount) from BO_mas where INVOICE_date<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "#  and tcurrency='USD' and AGENCY='" & findfirstfixup(lstCustomers) & "' and cancell='N'"
        Set rs15 = db.OpenRecordset(Sqlqry15, dbOpenDynaset)
        If IsNull(rs15.Fields(0)) = False Then Opcrsl = rs15.Fields(0)
        
        Sqlqry15 = "select Sum(Tra_namount) from BO_mas where INVOICE_date<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "#  and tcurrency='DHS' and AGENCY='" & findfirstfixup(lstCustomers) & "'and cancell='N'"
        Set rs15 = db.OpenRecordset(Sqlqry15, dbOpenDynaset)
        ...
        sqlqry16 = "select sum(TRA_amount) from crpr_mas where tdate<#" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# AND TCURRENCY = 'USD' and SUPP_NAME='" & findfirstfixup(lstCustomers) & "'"
        ...
        If rs18.RecordCount <> 0 Then
         ...
         Do Until rs18.EOF
           If rs18!tcurrency = "USD" Then
                ...
                ws.CommitTrans
           Else
                sqlqry19 = "Insert into CustReport values('" & findfirstfixup(lstCustomers) & "','" & rs18!vouc_no & "','" & rs18!vouc_type & "','" & Trim(rs18!tDate) & "','" & findfirstfixup(Trim(rs18!Description)) & "'," & 0 & "," & Val(rs18!tra_amount) / convertion & ")"
                ws.BeginTrans
                db.Execute (sqlqry19)
                ws.CommitTrans
           End If
        ...
        If rs19.RecordCount <> 0 Then
         ...
         Do Until rs19.EOF
          If rs19!tcurrency = "USD" Then
            ...
            ws.CommitTrans
          Else
            Sqlqry21 = "Insert into CustReport values('" & findfirstfixup(lstCustomers) & "','" & rs19!vouc_no & "','" & rs19!vouc_type & "','" & Trim(rs19!tDate) & "','" & findfirstfixup(Trim(rs19!Description)) & "'," & Val(rs19!tra_amount) / convertion & "," & 0 & ")"
            ws.BeginTrans
            db.Execute (Sqlqry21)
            ws.CommitTrans
          End If
        ...
        If rs20.RecordCount <> 0 Then
         ...
         Do Until rs20.EOF
           If rs20!tcurrency = "USD" Then
                ...
                ws.CommitTrans
           Else
                Sqlqry23 = "Insert into CustReport values('" & findfirstfixup(lstCustomers) & "','" & rs20!vouc_no & "','" & rs20!vouc_type & "','" & Trim(rs20!tDate) & "','" & findfirstfixup(Trim(rs20!Description)) & "'," & 0 & "," & Val(rs20!tra_amount) / convertion & ")"
                ws.BeginTrans
                db.Execute (Sqlqry23)
                ws.CommitTrans
            End If
        ...
        If rs21.RecordCount <> 0 Then
         ...
         Do Until rs21.EOF
           If rs21!tcurrency = "USD" Then
               ...
               ws.CommitTrans
           Else
              Sqlqry25 = "Insert into CustReport values('" & findfirstfixup(lstCustomers) & "','" & rs21!vouc_no & "','" & rs21!vouc_type & "','" & Trim(rs21!tDate) & "','" & findfirstfixup(Trim(rs21!Description)) & "'," & Val(rs21!tra_amount) / convertion & "," & 0 & ")"
               ws.BeginTrans
               db.Execute (Sqlqry25)
               ws.CommitTrans
           End If
          ...
          Do Until rs23.EOF
             If rs23!tcurrency = "USD" Then
                Sqlqry28 = "Insert into CustReport values('" & findfirstfixup(lstCustomers) & "','" & rs23!vouc_no & "','" & rs23!vouc_type & "','" & Trim(rs23!Cheque_Dt) & "','" & findfirstfixup(Trim(rs23!Description)) & "'," & 0 & "," & Val(rs23!tra_amount) & ")"
                 ...
                 ws.CommitTrans
             Else
                 Sqlqry28 = "Insert into CustReport values('" & findfirstfixup(lstCustomers) & "','" & rs23!vouc_no & "','" & rs23!vouc_type & "','" & Trim(rs23!Cheque_Dt) & "','" & findfirstfixup(Trim(rs23!Description)) & "'," & 0 & "," & Val(rs23!tra_amount) / convertion & ")"
                 ws.BeginTrans
                 db.Execute (Sqlqry28)
                 ws.CommitTrans
             End If
               ...
               rs25.MoveFirst
                Do Until rs25.EOF
                    If rs25!tcurrency = "USD" Then
                        ...
                        ws.CommitTrans
                    Else
                        Sqlqry31 = "Insert into CustReport values('" & findfirstfixup(lstCustomers) & "','" & rs25!vouc_no & "','" & rs25!vouc_type & "','" & Trim(rs24!Cheque_Dt) & "','" & findfirstfixup(Trim(rs25!Description)) & "'," & Val(rs25!tra_amount) / convertion & "," & 0 & ")"
                        ws.BeginTrans
                        db.Execute (Sqlqry31)
                        ws.CommitTrans
                    End If
         ...
         If rs26.RecordCount <> 0 Then
          ...
          Do Until rs26.EOF
           If rs26!tcurrency = "USD" Then
                ...
                ws.CommitTrans
           Else
                Sqlqry33 = "Insert into CustReport values('" & findfirstfixup(lstCustomers) & "','" & rs26!vouc_no & "','" & rs26!vouc_type & "','" & Trim(rs26!tDate) & "','" & findfirstfixup(Trim(rs26!Description)) & "'," & Val(rs26!tra_damount) / convertion & "," & 0 & ")"
                ws.BeginTrans
                db.Execute (Sqlqry33)
                ws.CommitTrans
           End If
    ...
    ' Journal Credit after From date and before to date
        ...
        Do Until rs27.EOF
          If rs27!tcurrency = "USD" Then
            ...
            ws.CommitTrans
           Else
            Sqlqry34 = "Insert into CustReport values('" & findfirstfixup(lstCustomers) & "','" & rs27!vouc_no & "','" & rs27!vouc_type & "','" & Trim(rs27!tDate) & "','" & findfirstfixup(Trim(rs27!Description)) & "'," & 0 & "," & Val(rs27!tra_Camount) / convertion & ")"
            ws.BeginTrans
            db.Execute (Sqlqry34)
            ws.CommitTrans
           End If
        ...
        If rs28.RecordCount <> 0 Then
         ...
         Do Until rs28.EOF
          If rs28!tcurrency = "USD" Then
            ...
            ws.CommitTrans
          Else
            Sqlqry36 = "Insert into CustReport values('" & findfirstfixup(lstCustomers) & "','" & rs28!vouc_no & "','" & rs28!vouc_type & "','" & Trim(rs28!tDate) & "','" & findfirstfixup(Trim(rs28!Description)) & "'," & 0 & "," & Val(rs28!tra_amount) / convertion & ")"
            ws.BeginTrans
            db.Execute (Sqlqry36)
            ws.CommitTrans
          End If
        ...
        If rs29.RecordCount <> 0 Then
         ...
         Do Until rs29.EOF
          If rs29!tcurrency = "USD" Then
            ...
            ws.CommitTrans
          Else
            Sqlqry38 = "Insert into CustReport values('" & findfirstfixup(lstCustomers) & "','" & rs29!vouc_no & "','" & rs29!vouc_type & "','" & Trim(rs29!tDate) & "','" & findfirstfixup(Trim(rs29!Description)) & "'," & Val(rs29!tra_amount) / convertion & "," & 0 & ")"
            ws.BeginTrans
            db.Execute (Sqlqry38)
            ws.CommitTrans
          End If
        ...
        Set rs30 = db.OpenRecordset(Sqlqry38, dbOpenDynaset)
         ...
         Do Until rs30.EOF
           If rs30!tcurrency = "USD" Then
                ...
                ws.CommitTrans
           Else
                Sqlqry39 = "Insert into CustReport values('" & findfirstfixup(lstCustomers) & "','" & rs30!vouc_no & "','" & rs30!vouc_type & "','" & Trim(rs30!tDate) & "','" & findfirstfixup(Trim(rs30!Description)) & "'," & 0 & "," & Val(rs30!tra_amount) / convertion & ")"
                ws.BeginTrans
                db.Execute (Sqlqry39)
                ws.CommitTrans
           End If
    ...
    ' Credit Note - Debit after From date and before to date
        ...
        Do Until rs31.EOF
          If rs31!tcurrency = "USD" Then
            ...
            ws.CommitTrans
          Else
            Sqlqry41 = "Insert into CustReport values('" & findfirstfixup(lstCustomers) & "','" & rs31!vouc_no & "','" & rs31!vouc_type & "','" & Trim(rs31!tDate) & "','" & findfirstfixup(Trim(rs31!Description)) & "'," & Val(rs31!tra_amount) / convertion & "," & 0 & ")"
            ws.BeginTrans
            db.Execute (Sqlqry41)
            ws.CommitTrans
          End If
    ...
    ' Credit Sale after From date and before to date
        ...
        Do Until rs32.EOF
          If rs32!tcurrency = "USD" Then
            Sqlqry43 = "Insert into CustReport values('" & findfirstfixup(lstCustomers) & "','" & rs32!serial_no & "','INV','" & Trim(rs32!invoice_date) & "','" & findfirstfixup(Trim(rs32!bo_ref)) & " " & Trim(rs32!media) & "'," & Trim(rs32!tra_namount) & "," & 0 & ")"
            ws.BeginTrans
            db.Execute (Sqlqry43)
            ws.CommitTrans
          Else
            Sqlqry43 = "Insert into CustReport values('" & findfirstfixup(lstCustomers) & "','" & rs32!serial_no & "','INV','" & Trim(rs32!invoice_date) & "','" & findfirstfixup(Trim(rs32!bo_ref)) & " " & Trim(rs32!media) & "'," & Val(rs32!tra_namount) / convertion & "," & 0 & ")"
            ws.BeginTrans
            db.Execute (Sqlqry43)
            ws.CommitTrans
          End If
        ...
        If rs33.RecordCount <> 0 Then
         ...
         Do Until rs33.EOF
           If rs33!tcurrency = "USD" Then
                ...
                ws.CommitTrans
            Else
                Sqlqry45 = "Insert into CustReport values('" & findfirstfixup(lstCustomers) & "','" & rs33!vouc_no & "','" & rs33!vouc_type & "','" & Trim(rs33!tDate) & "','Purchases'," & 0 & "," & Val(rs33!tra_amount) / convertion & ")"
                ws.BeginTrans
                db.Execute (Sqlqry45)
                ws.CommitTrans
            End If
   ...
   ' Pdc Receipts after From date and before to date
    ...
    With CrystalReport1
     ...
     .ReportFileName = App.Path & "\CusReport.rpt"
     .Formulas(0) = "zzz='" & " As on " & Trim(txtdateto.TextWithMask) & "'"
   ' .Formulas(1) = "yyy='" & Mid(lstCustomers, 1, 35) & "'"
...
Set ws = DBEngine.Workspaces(0)
Set db = ws.OpenDatabase(App.Path & "\misov.mdb")
Sqlqry1 = "Select * from agndtls order by agentname"
