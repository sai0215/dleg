...
Begin VB.Form frmDebitNoteAdd 
   BackColor       =   &H00FFFFC0&
   Caption         =   "Debit Note Addition"
   ClientHeight    =   8775
   ClientLeft      =   -60
   ClientTop       =   285
   ClientWidth     =   12060
   ...
   End
End
Attribute VB_Name = "frmDebitNoteAdd"
...
Dim ctype As String
 If ValidateData = True Then
    ...
    Set db = ws.OpenDatabase(App.Path & "\misov.mdb")
       Sqlqry = " Insert into debt_mas values('" & lblVoucNo & "','DNT','" _
                                     & Format(txtdate.TextWithMask, "dd/mm/yyyy") & "','" _
                                     & Mid(lstCreditedTo, 1, 6) & "','" _
                                     & Mid(lstCreditedTo, 12, 35) & "','" _
                                     & Mid(lstDebitedTo, 1, 4) & "','" _
                                     & Trim(Mid(lstDebitedTo, 9, 35)) & "','" _
     ...
     Set ws = DBEngine.Workspaces(0)
     Set db = ws.OpenDatabase(App.Path & "\misov.mdb")
     Sqlqry = "Update docu_mas set doc_no='" & lblVoucNo & "' where doc_type='DNT'"
     ws.BeginTrans
     db.Execute (Sqlqry)
     ws.CommitTrans
  ...
  If X = vbYes Then
   If ctype = "DHS" Then
        ...
        CrystalReport1.SelectionFormula = "{Debt_Mas.Vouc_no}=" & Val(lblVoucNo.Caption) - 1 & ""
...
Set ws = DBEngine.Workspaces(0)
Set db = ws.OpenDatabase(App.Path & "\misov.mdb")
Sqlqry = "Select DOC_no from DOCU_MAS WHERE DOC_TYPE='DNT'"
Set rs = db.OpenRecordset(Sqlqry, dbOpenDynaset)
If rs.RecordCount = 0 Then
   MsgBox "Document type 'DNT' not found"
   Exit Sub
Else
   rs.MoveLast
