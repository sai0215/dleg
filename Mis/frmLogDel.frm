VERSION 5.00
Begin VB.Form frmLogDel 
   BackColor       =   &H00C0C0FF&
   Caption         =   "Login To Delete"
   ClientHeight    =   3720
   ClientLeft      =   3060
   ClientTop       =   2505
   ClientWidth     =   5055
...
If Forms.Count = 2 Then MainForm.Picture2.Visible = True
If Cancel = 1 Then Unload Me
End Sub
...
Private Sub ChangePassword()
    ...
    If StrComp(Trim(txtOldPwd), logString, vbTextCompare) = 0 Then
        If StrComp(Trim(txtNewPwd), Trim(txtConfirmPwd), vbTextCompare) = 0 Then
            Sqlqry = "Update PWD Set LogString='" & Trim(txtConfirmPwd) & "'"
            ws.BeginTrans
            db.Execute Sqlqry
            ws.CommitTrans
...
Private Sub cmdOK_Click()
    ...
    Set rs = db.OpenRecordset(Sqlqry, dbOpenDynaset)
    If rs.RecordCount = 0 Then
        Sqlqry = "Insert into PWD Values('" & Trim(txtPassword) & "')"
        ws.BeginTrans
        db.Execute Sqlqry
        ws.CommitTrans
    Else
