...
Begin VB.Form frmLoginreports 
   ...
   End
End
Attribute VB_Name = "frmLoginreports"
 ...
 Set db = ws.OpenDatabase(App.Path & "\misov.mdb")
    ...
    If StrComp(Trim(txtOldPwd), logString, vbTextCompare) = 0 Then
        If StrComp(Trim(txtNewPwd), Trim(txtConfirmPwd), vbTextCompare) = 0 Then
            Sqlqry = "Update PWDLOGREP Set PWD='" & Trim(txtConfirmPwd) & "'"
            ws.BeginTrans
            db.Execute Sqlqry
            ws.CommitTrans
...
Private Sub cmdOK_Click()
  ...
  Set db = ws.OpenDatabase(App.Path & "\misov.mdb")
    ...
    Set rs = db.OpenRecordset(Sqlqry, dbOpenDynaset)
    If rs.RecordCount = 0 Then
        Sqlqry = "Insert into PWDLOGREP Values('" & Trim(txtPassword) & "')"
        ws.BeginTrans
        db.Execute Sqlqry
        ws.CommitTrans
    Else
