...
Begin VB.Form frmSaleRepMedia 
   BackColor       =   &H00FFFFC0&
   Caption         =   "Sales Report - Media Wise"
   ClientHeight    =   8775
   ClientLeft      =   -45
   ClientTop       =   330
   ClientWidth     =   11985
   ...
   Begin VB.Frame Frame1 
      BackColor       =   &H00FFFFC0&
      Caption         =   "Invoice List - Media Wise"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   12
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmSaleRepMedia"
 ...
 If ValidateData = True Then
    ...
    Set db = ws.OpenDatabase(App.Path & "\misov.mdb")
     Sqlqry = " Delete * from invrep"
       ...
       If Mid(lstMedia, 1, 10) = "Magazine :" Then
       
              Sqlqry1 = "Select * from bo_mas where Invoice_date>= #" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# and Invoice_date<=#" & DateValue(Format(txtdateto.TextWithMask, "dd/mm/yyyy")) & "# and Media='Magazine' and sub_Media='" & Trim(Mid(lstMedia, 12, 20)) & "' and status='N'"
          ...
          Else
             Sqlqry1 = "Select * from bo_mas where Invoice_date>= #" & DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy")) & "# and Invoice_date<=#" & DateValue(Format(txtdateto.TextWithMask, "dd/mm/yyyy")) & "# and Media='" & Trim(lstMedia) & "' and status='N'"
             Set rs = db.OpenRecordset(Sqlqry1, dbOpenDynaset)
             If rs.RecordCount <> 0 Then
               rs.MoveFirst
               Do Until rs.EOF
...
Private Sub populateMedia()
    ...
    lstMedia.Clear
     ...
     If rs.RecordCount = 0 Then
          MsgBox "No Records found in the Media Register"
     Else
          rs.MoveFirst
          lstMedia.AddItem "Cinema"
          lstMedia.AddItem "Magazine"
          lstMedia.AddItem "Online"
          lstMedia.AddItem "Television"
       Do Until rs.EOF
          lstMedia.AddItem "Magazine" & " : " & rs!sub_media
          rs.MoveNext
       Loop
     End If

End Sub
Private Sub lstmedia_KeyPress(KeyAscii As Integer)
...
If IsDate(DateValue(Format(txtdatefrom.TextWithMask, "dd/mm/yyyy"))) = False Then
   ...
   SendKeys " {Home} + {End} "
   Exit Function
ElseIf lstMedia.SelCount = 0 Then
  MsgBox "Select Media", vbInformation, "Invalid Entry"
  lstMedia.SetFocus
  SendKeys " {Home} + {end} "
  Exit Function
